...
public class SQLStatement {
    ...
    public static final String GET_GIFT_CODE = "SELECT * FROM `gift_codes` WHERE `code` = ? AND (`server_id` = ? OR `server_id` = 0) AND `disabled` = 0 AND (starts_at IS NULL OR starts_at <= now()) AND (expires_at IS NULL OR expires_at > now()) LIMIT 1 FOR UPDATE;";
    public static final String UPDATE_GIFT_CODE = "UPDATE `gift_codes` SET `status` = CASE WHEN `type` = 0 THEN 1 ELSE `status` END, `redemption_count` = `redemption_count` + 1, `updated_at` = ? WHERE `id` = ? LIMIT 1;";
