...
const user = readFileSync(new URL("../src/main/java/Exe_Z/model/User.java", import.meta.url), "utf8");
...
describe("Shinwa expiry mailbox notification", () => {
  it("bootstraps a marker table keyed by listing and server", () => {
    ...
    expect(server).toContain("panel_player_inbox");
    expect(server).toContain("PRIMARY KEY (shinwa_id, server_id)");
    expect(readFileSync(new URL("../SQL/nsoz.sql", import.meta.url), "utf8")).toContain("panel_player_inbox");
  });
  ...
  it("claims expiry notification atomically before writing the seller mailbox", () => {
    ...
    expect(manager).toContain("INSERT INTO panel_player_inbox");
    expect(manager).toContain("delivery_status");
    expect(manager).toContain("dedupeKey");
    expect(manager).toContain("source_type");
  });

  it("notifies online seller immediately and keeps offline seller notification pending", () => {
    expect(manager).toContain("ServerManager.findCharById");
    ...
    expect(user).toContain("deliverPendingInbox");
